                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            <form action="{{ route('admin.comics.approved', $comic) }}" method="post">
                                @csrf
                                <button type="submit" class="bg-green-500 text-white font-bold p-2 rounded-full">
                                    Aprovar comic
                                </button>
                            </form>
                        </div>
                        ...
                        <div>
                            @livewire('admin.observations', ['comic' => $comic])
                        </div>
                    </div>
                </div>
                <div class="my-7">
                    ...
                    @foreach ($chapters as $chapter)
                        <div class="p-2 ">
                            <div class="bg-gray-200 p-2 w-full rounded-lg flex justify-between items-center">
                                <h1 class="text-lg font-bold font-josefin">{{ $chapter->position }}.
                                    {{ $chapter->name }}
                                </h1>
                                <div class="">
